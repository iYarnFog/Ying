package com.zyy.YingNMS.YingParticle;
...
import com.zyy.YingNMS.YingReflection;
...
public enum YingParticleEffect
{
  ...
  private static final Map<String, YingParticleEffect> NAME_MAP;
  private static final Map<Integer, YingParticleEffect> ID_MAP;
  ...
  static
  {
    ...
    ID_MAP = new HashMap();
    for (YingParticleEffect effect : values())
    {
  ...
  private YingParticleEffect(String name, int id, int requiredVersion, ParticleProperty... properties)
  {
    this.name = name;
    this.id = id;
  ...
  public static YingParticleEffect fromName(String name)
  {
    for (Map.Entry<String, YingParticleEffect> entry : NAME_MAP.entrySet()) {
      if (((String)entry.getKey()).equalsIgnoreCase(name)) {
        return (YingParticleEffect)entry.getValue();
      }
    }
    return null;
  }
  
  public static YingParticleEffect fromId(int id)
  {
    for (Map.Entry<Integer, YingParticleEffect> entry : ID_MAP.entrySet()) {
  ...
  private static boolean isDataCorrect(YingParticleEffect effect, ParticleData data)
  {
    return ((effect != BLOCK_CRACK) && (effect != BLOCK_DUST)) || (((data instanceof BlockData)) || ((effect == ITEM_CRACK) && ((data instanceof ItemData))));
  }
  
  private static boolean isColorCorrect(YingParticleEffect effect, ParticleColor color)
  {
  ...
  public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, double range)
    throws YingParticleEffect.ParticleVersionException, YingParticleEffect.ParticleDataException, IllegalArgumentException
  {
    if (!isSupported()) {
      throw new ParticleVersionException("This YingParticle effect is not supported by your server version");
    }
    if (hasProperty(ParticleProperty.REQUIRES_DATA)) {
      throw new ParticleDataException("This YingParticle effect requires additional data");
    }
    ...
    throws YingParticleEffect.ParticleVersionException, YingParticleEffect.ParticleDataException, IllegalArgumentException
  {
    ...
    if (!hasProperty(ParticleProperty.DIRECTIONAL)) {
      throw new IllegalArgumentException("This YingParticle effect is not directional");
    }
    if ((hasProperty(ParticleProperty.REQUIRES_WATER)) && (!isWater(center))) {
      throw new IllegalArgumentException("There is no water at the center location");
    }
  ...
  public void display(ParticleColor color, Location center, double range)
    throws YingParticleEffect.ParticleVersionException, YingParticleEffect.ParticleColorException
  {
    if (!isSupported()) {
      throw new ParticleVersionException("This YingParticle effect is not supported by your server version");
    }
    if (!hasProperty(ParticleProperty.COLORABLE)) {
      throw new ParticleColorException("This YingParticle effect is not colorable");
    }
    if (!isColorCorrect(this, color)) {
      throw new ParticleColorException("The YingParticle color type is incorrect");
    }
  ...
  public void display(ParticleData data, float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, double range)
    throws YingParticleEffect.ParticleVersionException, YingParticleEffect.ParticleDataException
  {
    if (!isSupported()) {
      throw new ParticleVersionException("This YingParticle effect is not supported by your server version");
    }
    if (!hasProperty(ParticleProperty.REQUIRES_DATA)) {
      throw new ParticleDataException("This YingParticle effect does not require additional data");
    }
    if (!isDataCorrect(this, data)) {
      throw new ParticleDataException("The YingParticle data type is incorrect");
    }
  ...
  public static final class ItemData
    extends YingParticleEffect.ParticleData
  {
  ...
  public static final class OrdinaryColor
    extends YingParticleEffect.ParticleColor
  {
  ...
  public static final class ParticlePacket
  {
    ...
    private final YingParticleEffect effect;
    ...
    private final YingParticleEffect.ParticleData data;
    private Object packet;
    
    public ParticlePacket(YingParticleEffect effect, float offsetX, float offsetY, float offsetZ, float speed, int amount, boolean longDistance, YingParticleEffect.ParticleData data)
      throws IllegalArgumentException
    {
      initialize();
      if (speed < 0.0F) {
    ...
    public ParticlePacket(YingParticleEffect effect, Vector direction, float speed, boolean longDistance, YingParticleEffect.ParticleData data)
      throws IllegalArgumentException
    {
      this(effect, (float)direction.getX(), (float)direction.getY(), (float)direction.getZ(), speed, 0, longDistance, data);
    }
    
    public ParticlePacket(YingParticleEffect effect, YingParticleEffect.ParticleColor color, boolean longDistance)
    {
      this(effect, color.getValueX(), color.getValueY(), color.getValueZ(), 1.0F, 0, longDistance, null);
      if ((effect == YingParticleEffect.REDSTONE) && ((color instanceof YingParticleEffect.OrdinaryColor)) &&
        (((YingParticleEffect.OrdinaryColor)color).getRed() == 0)) {
        this.offsetX = 1.175494E-038F;
      }
    }
    
    public static void initialize()
      throws YingParticleEffect.ParticlePacket.VersionIncompatibleException
    {
      if (initialized) {
        return;
      }
      try
      {
        version = Integer.parseInt(YingReflection.PackageType.getServerVersion().split("_")[1]);
        if (version > 7) {
          enumParticle = YingReflection.PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
        }
        Class<?> packetClass = YingReflection.PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles" : "PacketPlayOutWorldParticles");
        packetConstructor = YingReflection.getConstructor(packetClass, new Class[0]);
        getHandle = YingReflection.getMethod("CraftPlayer", YingReflection.PackageType.CRAFTBUKKIT_ENTITY, "getHandle", new Class[0]);
        playerConnection = YingReflection.getField("EntityPlayer", YingReflection.PackageType.MINECRAFT_SERVER, false, "playerConnection");
        
        sendPacket = YingReflection.getMethod(playerConnection.getType(), "sendPacket", new Class[] {YingReflection.PackageType.MINECRAFT_SERVER
          .getClass("Packet") });
      }
    ...
    private void initializePacket(Location center)
      throws YingParticleEffect.ParticlePacket.PacketInstantiationException
    {
      if (this.packet != null) {
        return;
      }
      try
      {
        ...
        if (version < 8)
        {
          ...
          YingReflection.setValue(this.packet, true, "a", name);
        }
        else
        {
          YingReflection.setValue(this.packet, true, "a", enumParticle.getEnumConstants()[this.effect.getId()]);
          YingReflection.setValue(this.packet, true, "j", Boolean.valueOf(this.longDistance));
          if (this.data != null)
          {
            int[] packetData = this.data.getPacketData();
            YingReflection.setValue(this.packet, true, "k", new int[] { this.effect == YingParticleEffect.ITEM_CRACK ? packetData[0] : packetData[0] | packetData[1] << 12 });
          }
        }
        YingReflection.setValue(this.packet, true, "b", Float.valueOf((float)center.getX()));
        YingReflection.setValue(this.packet, true, "c", Float.valueOf((float)center.getY()));
        YingReflection.setValue(this.packet, true, "d", Float.valueOf((float)center.getZ()));
        YingReflection.setValue(this.packet, true, "e", Float.valueOf(this.offsetX));
        YingReflection.setValue(this.packet, true, "f", Float.valueOf(this.offsetY));
        YingReflection.setValue(this.packet, true, "g", Float.valueOf(this.offsetZ));
        YingReflection.setValue(this.packet, true, "h", Float.valueOf(this.speed));
        YingReflection.setValue(this.packet, true, "i", Integer.valueOf(this.amount));
      }
      catch (Exception exception)
      {
    ...
    public void sendTo(Location center, Player player)
      throws YingParticleEffect.ParticlePacket.PacketInstantiationException, YingParticleEffect.ParticlePacket.PacketSendingException
    {
