 ...
 * official release.
 */
public class YingCompatibility extends YingNMS {
    ...
    public static List<Entity> getNearbyEntities(Location location, double x, double y, double z) {
        ...
        Object worldHandle = getHandle(location.getWorld());
        try {
            x = Math.min(x, YingCompatibility.MAX_ENTITY_RANGE);
            z = Math.min(z, YingCompatibility.MAX_ENTITY_RANGE);
    ...
    public static Projectile spawnProjectile(Class<?> projectileType, Location location, Vector direction, ProjectileSource source, float speed, float spread, float spreadLocations, Random random) {
        ...
        try {
            ...
            Entity entity = YingNMS.getBukkitEntity(nmsProjectile);
    ...
    public static void applyItemData(ItemStack item, Block block) {
        try {
            ...
            YingNMS.setTileEntityData(block.getLocation(), entityDataTag);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    //private static final int MINOR_VERSION_NUMBER = Integer.parseInt(YingReflection.PackageType.getServerVersion().split("_")[1]);



    public static void YingChatPacket(Player YPlayer, String YJson, byte YType) throws ReflectiveOperationException
    {
        Object craftPlayer = YingReflection.PackageType.CRAFTBUKKIT.getClass("entity.CraftPlayer").cast(YPlayer);

        Object craftHandle = YingReflection.PackageType.CRAFTBUKKIT.getClass("entity.CraftPlayer").getMethod("getHandle", new Class[0]).invoke(craftPlayer, new Object[0]);

        Object playerConnection = YingReflection.PackageType.MINECRAFT_SERVER.getClass("EntityPlayer").getField("playerConnection").get(craftHandle);
        Object parsedMessage;
        if (YingReflection.PackageType.getServerVersion().startsWith("1_8_R1")) {
            parsedMessage = YingReflection.PackageType.MINECRAFT_SERVER.getClass("ChatSerializer").getMethod("a", new Class[] { String.class }).invoke(null, new Object[] { ChatColor.translateAlternateColorCodes('&', YJson) });
        } else {
            parsedMessage = Class.forName(YingReflection.PackageType.MINECRAFT_SERVER + "." + "IChatBaseComponent" + "$" + "ChatSerializer").getMethod("a", new Class[] { String.class }).invoke(null, new Object[] { ChatColor.translateAlternateColorCodes('&', YJson) });
        }
        Object packetPlayOutChat;
        if (Integer.parseInt(YingReflection.PackageType.getServerVersion().split("_")[1]) < 12)
        {
            packetPlayOutChat = YingReflection.PackageType.MINECRAFT_SERVER.getClass("PacketPlayOutChat").getConstructor(new Class[] { YingReflection.PackageType.MINECRAFT_SERVER.getClass("IChatBaseComponent"), Byte.TYPE }).newInstance(new Object[] { parsedMessage, Byte.valueOf(YType) });
        }
        else
        {
            Class<?> chatMessageTypeClass = YingReflection.PackageType.MINECRAFT_SERVER.getClass("ChatMessageType");
            Enum<?> chatType = null;
            for (Object chatMessageType : chatMessageTypeClass.getEnumConstants())
            {
                Enum<?> e = (Enum)chatMessageType;
                if ((("SYSTEM".equalsIgnoreCase(e.name())) && (YType == 1)) || (("GAME_INFO".equalsIgnoreCase(e.name())) && (YType == 2)))
                {
                    chatType = e;
                    break;
                }
            }
            packetPlayOutChat = YingReflection.PackageType.MINECRAFT_SERVER.getClass("PacketPlayOutChat").getConstructor(new Class[] { YingReflection.PackageType.MINECRAFT_SERVER.getClass("IChatBaseComponent"), chatMessageTypeClass }).newInstance(new Object[] { parsedMessage, chatType });
        }
        YingReflection.PackageType.MINECRAFT_SERVER.getClass("PlayerConnection").getMethod("sendPacket", new Class[] { YingReflection.PackageType.MINECRAFT_SERVER.getClass("Packet") }).invoke(playerConnection, new Object[] { packetPlayOutChat });
    }
